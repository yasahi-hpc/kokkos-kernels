...
struct SerialLaswp<Side::Left, Direct::Forward> {
  ...
  KOKKOS_INLINE_FUNCTION static int invoke(const PivViewType piv,
    ...
    if constexpr (AViewType::rank == 1) {
      ...
      SerialLaswpVectorForwardInternal::invoke(plen, piv.data(), ps0, A.data(),
                                               as0);
    } else if constexpr (AViewType::rank == 2) {
      // row permutation
      const int plen = piv.extent(0), ps0 = piv.stride(0), n = A.extent(1),
                as0 = A.stride(0), as1 = A.stride(1);
      SerialLaswpMatrixForwardInternal::invoke(n, plen, piv.data(), ps0,
                                               A.data(), as0, as1);
    }
    return 0;
  }
...
struct SerialLaswp<Side::Right, Direct::Forward> {
  ...
  KOKKOS_INLINE_FUNCTION static int invoke(const PivViewType &piv,
    ...
    if constexpr (AViewType::rank == 1) {
      ...
      SerialLaswpVectorForwardInternal ::invoke(plen, piv.data(), ps, A.data(),
                                                as0);
    } else if constexpr (AViewType::rank == 2) {
      // column permutation
      const int plen = piv.extent(0), ps = piv.stride(0), m = A.extent(0),
                as0 = A.stride(0), as1 = A.stride(1);
      SerialLaswpMatrixForwardInternal ::invoke(m, plen, piv.data(), ps,
                                                A.data(), as1, as0);
    }
    return 0;
  }
...
struct SerialLaswp<Side::Left, Direct::Backward> {
  ...
  KOKKOS_INLINE_FUNCTION static int invoke(const PivViewType piv,
    ...
    if constexpr (AViewType::rank == 1) {
      const int plen = piv.extent(0), ps0 = piv.stride(0), as0 = A.stride(0);
      SerialLaswpVectorBackwardInternal::invoke(plen, piv.data(), ps0, A.data(),
                                                as0);
    } else if constexpr (AViewType::rank == 2) {
      // row permutation
      const int plen = piv.extent(0), ps0 = piv.stride(0), n = A.extent(1),
                as0 = A.stride(0), as1 = A.stride(1);
      SerialLaswpMatrixBackwardInternal::invoke(n, plen, piv.data(), ps0,
                                                A.data(), as0, as1);
    }
    return 0;
  }
...
struct SerialLaswp<Side::Right, Direct::Backward> {
  ...
  KOKKOS_INLINE_FUNCTION static int invoke(const PivViewType &piv,
    ...
    if constexpr (AViewType::rank == 1) {
      const int plen = piv.extent(0), ps = piv.stride(0), as0 = A.stride(0);
      SerialLaswpVectorBackwardInternal ::invoke(plen, piv.data(), ps, A.data(),
                                                 as0);
    } else if constexpr (AViewType::rank == 2) {
      // column permutation
      const int plen = piv.extent(0), ps = piv.stride(0), m = A.extent(0),
                as0 = A.stride(0), as1 = A.stride(1);
      SerialLaswpMatrixBackwardInternal ::invoke(m, plen, piv.data(), ps,
                                                 A.data(), as1, as0);
    }
    return 0;
  }
