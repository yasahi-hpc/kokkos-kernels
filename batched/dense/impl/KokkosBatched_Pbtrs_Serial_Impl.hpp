...
KOKKOS_INLINE_FUNCTION static int checkPbtrsInput(
    [[maybe_unused]] const AViewType &A, [[maybe_unused]] const XViewType &x) {
  static_assert(Kokkos::is_view<AViewType>::value,
                "KokkosBatched::pbtrs: AViewType is not a Kokkos::View.");
  static_assert(Kokkos::is_view<XViewType>::value,
                "KokkosBatched::pbtrs: XViewType is not a Kokkos::View.");
  static_assert(AViewType::rank == 2,
                "KokkosBatched::pbtrs: AViewType must have rank 2.");
  static_assert(XViewType::rank == 1,
                "KokkosBatched::pbtrs: XViewType must have rank 1.");

#if (KOKKOSKERNELS_DEBUG_LEVEL > 0)
  const int ldb  = x.extent(0);
  const int ldab = A.extent(0), n = A.extent(1);
